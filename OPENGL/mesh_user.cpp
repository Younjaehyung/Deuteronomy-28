...
void Material::SetToProgram ( const Program* program ) const {
    ...
    if ( diffuse ) {
        ...
        program->SetUniform ( "material.diffuse" , textureCount );  //material.diffuse 설정
        diffuse->Bind ( );
        textureCount++;
    }
    ...
    if ( specular ) {
        glActiveTexture ( GL_TEXTURE0 + textureCount );
        program->SetUniform ( "material.specular" , textureCount ); //material.specular 설정
        specular->Bind ( );
        textureCount++;
    }

    glActiveTexture ( GL_TEXTURE0 );    //텍스트 유닛 초기화

    program->SetUniform ( "material.shininess" , shininess );    //material.shininess 설정
}