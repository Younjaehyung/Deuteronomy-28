...
void Camera::Update ( ) {
	...
	if ( moving_Time==0.0f && Ismoving ) {

		moving_Time += Time::DeltaTime ( );
		m_camerRoll = 2.0f;
	}
	else if ( Ismoving && moving_Time>0.5f ) {
		m_camerRoll = -1.0f * m_camerRoll;
		moving_Time += Time::DeltaTime ( );
		moving_Time -= 0.5f;
	}
	else if (!Ismoving ){
	...
	m_cameraFront =
		glm::rotate ( glm::mat4 ( 1.0f ) , glm::radians ( m_camerRoll ) , glm::vec3 ( 0.0f , 0.0f , 0.1f ) )
